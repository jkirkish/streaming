...
public class SuspectFinder {
	...
	public static void main(String[] args) {
		...
		Map<Integer, List<Suspect>> suspects = suspectLineUp.stream()
															.collect(Collectors.groupingBy((criminal)-> criminal.getBirthYear()));
		                                              
															
		 Map<Integer, List<Suspect>> who = suspects.entrySet()
	        									   .stream()
	        									   .map(entry -> entry.getValue())
	        									   .filter(hair ->hair.get(0).getHairColor().equals("RED"))
	        									   .filter(glasses ->glasses.get(0).getHasGlasses().equals(true));
